...
CREATE TABLE formacion_que_recibe_ficha ( 
	id_formacion_recibe_ficha int NOT NULL ,
	 ...
	 Numero_de_ficha int NOT NULL, 
	Resultado_de_aprendizaje boolean NOT NULL,
 PRIMARY key(id_formacion_recibe_ficha), FOREIGN key(id_ruta_de_aprendizaje) REFERENCES gestión_de_ruta_de_aprendizaje(id_ruta_aprendizaje) on DELETE CASCADE ON UPDATE CASCADE, FOREIGN key( Id_GestionDeFichas) REFERENCES gestion_de_fichas( Id_GestionDeFichas ) on DELETE CASCADE ON UPDATE CASCADE )ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE administrar_perfil (
  ...
  estado_perfil boolean NOT NULL default 1,
...
CREATE TABLE asignacion_planificacion_formacion_instructor ( 
    ...
    FOREIGN key (id_administrar_perfil) REFERENCES administrar_perfil(id_administrar_perfil) on DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN key( id_ruta_aprendizaje) REFERENCES gestión_de_ruta_de_aprendizaje(id_ruta_aprendizaje) on DELETE CASCADE ON UPDATE CASCADE,
